...
test('Should handle POST with empty/nonsensical body', async () => {
    ...
    const app = (0, app_1.createApp)({ proxySecrets, unleashUrl, unleashApiToken }, client);
    client.emit('ready');
    await Promise.all([{ blah: 'hello' }, undefined, {}].map((body) => (0, supertest_1.default)(app)
        .post('/proxy')
        .type('json')
        .send(body)
...
test('Should return list of toggles using env with multiple secrets', () => {
    ...
    const client = new client_mock_1.default(toggles);
    const app = (0, app_1.createApp)({ unleashUrl, unleashApiToken }, client);
    client.emit('ready');
    return (0, supertest_1.default)(app)
        .get('/proxy')
...
test('Should send in context with ip as remoteAddress', async () => {
    ...
    const client = new client_mock_1.default(toggles);
    const proxySecrets = ['sdf'];
    const app = (0, app_1.createApp)({ proxySecrets, unleashUrl, unleashApiToken, trustProxy: true }, client);
    client.emit('ready');
    await (0, supertest_1.default)(app)
        .get('/proxy?userId=123&tenantId=me')
        .set('Authorization', 'sdf')
...
test('Should return the same origin based on cors options', async () => {
    ...
    const response = await (0, supertest_1.default)(app)
        ...
        .expect(200)
        .expect('Content-Type', /json/);
    expect(response.headers).toHaveProperty('access-control-allow-origin', 'https://demo.unleash-hosted.com');
});
...
test('Should return all feature toggles', () => {
    ...
    const proxySecrets = ['sdf'];
    const app = (0, app_1.createApp)({ unleashUrl, unleashApiToken, proxySecrets, enableAllEndpoint: true }, client);
    client.emit('ready');
    return (0, supertest_1.default)(app)
        .get('/proxy/all')
        .set('Authorization', 'sdf')
